...
## L'arbre de passifs (P), plein cadre. Glisser déplace la vue ; clic gauche prend,
## clic droit reprend, comme sur la page d'un manuel. Les règles sont dans
## `PassiveTree` : ce panneau n'en vérifie aucune, il colore ce qu'elles répondent.
...
## Pixels par case de la grille de l'arbre : les 38 cases de haut de l'arbre tiennent
## presque dans le cadrage, un déplacement montre le reste.
const UNIT := 10.0
...
func screen_position(n: PassiveNode) -> Vector2:
	return size * 0.5 + _pan + Vector2(n.position) * UNIT


func node_at(point: Vector2) -> PassiveNode:
	for n in _player.passive_tree.nodes:
		if screen_position(n).distance_to(point) <= _radius(n) + 2.0:
			return n
	return null
...
func _draw() -> void:
	...
	draw_string(
		...
		Texts.t("[clic] prendre     [clic droit] reprendre     [glisser] déplacer     P pour fermer"),
...
func _radius(n: PassiveNode) -> float:
	match n.kind:
		PassiveNode.Kind.NOTABLE, PassiveNode.Kind.START:
			return NOTABLE_RADIUS
		PassiveNode.Kind.KEYSTONE:
			return KEYSTONE_RADIUS
	return SMALL_RADIUS
...
func _draw_node(n: PassiveNode) -> void:
	...
	var icon := PassiveIcon.texture(n)
	if icon != null:
