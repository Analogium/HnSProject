...
func test_the_enemy_hurtbox_carries_its_sheet() -> void:
...
func test_affixing_does_not_touch_the_shared_resource() -> void:
	var disk: CharacterStats = load("res://resources/stats/grunt_stats.tres")
	var before := disk.max_health
	for i in 12:
		var e: Enemy = load("res://actors/enemies/grunt.tscn").instantiate()
		e.position = Vector2(i * 37, i * 53)   # le tirage se déduit de la case
		add_child_autofree(e)
	assert_eq(disk.max_health, before, "grunt_stats.tres est intact")


func test_armored_gives_armor() -> void:
	var armored: Affix = load("res://resources/affixes/armored.tres")
	assert_gt(armored.armor, 0.0)


## Nulle par défaut, la régénération ne doit rien faire du tout.
func test_without_regeneration_health_does_not_move() -> void:
	var e: Enemy = load("res://actors/enemies/grunt.tscn").instantiate()
	add_child_autofree(e)
	var hp := e.health
	e.regen(1.0)
	assert_eq(e.health, hp)


func test_the_manager_adds_and_removes() -> void:
...
func test_an_enemy_spawns_at_its_zone_level() -> void:
	var m := EnemyManager.new()
	m.level = 40
	add_child_autofree(m)
	var scene: PackedScene = load("res://actors/enemies/grunt.tscn")
	...
	assert_eq(e.level, 40)
	assert_gt(e.stats.max_health, weak.max_health * 7.0, "huit fois la vie, affixes en plus")
...
## écrivain.
func test_scaling_does_not_touch_the_shared_resource() -> void:
	var disk: CharacterStats = load("res://resources/stats/grunt_stats.tres")
	var before := disk.max_health
	var m := EnemyManager.new()
	m.level = 55
...
func test_without_a_set_level_the_enemy_stays_at_the_first() -> void:
	var m := EnemyManager.new()
	add_child_autofree(m)
	var e := m.spawn(load("res://actors/enemies/grunt.tscn"), Vector2(300, 300))
	var disk: CharacterStats = load("res://resources/stats/grunt_stats.tres")
	assert_eq(e.level, 1)
	assert_almost_eq(e.stats.max_health, disk.max_health, 0.001)
...
func test_rack_manuals_learn_from_each_death() -> void:
	var scene := await _kill_scene()
	var player: Player = scene[0]
	var manager: EnemyManager = scene[1]
	var grunt: Enemy = scene[2]

	var studied := _book()
	player.rack.put(0, studied)
	...
	assert_gt(studied.manual.experience, 0, "le livre à l'étude a appris")
	assert_eq(
		studied.manual.experience, player.xp,
...
## choix des trois.
func test_a_manual_in_the_bag_learns_nothing() -> void:
	var scene := await _kill_scene()
	var player: Player = scene[0]
	var manager: EnemyManager = scene[1]
	var grunt: Enemy = scene[2]
	...
	assert_eq(range.manual.experience, 0)
	assert_gt(player.xp, 0, "le personnage, lui, a bien gagné")


## Trois manuels reçoivent chacun le tout : un deuxième livre doit être une
## ouverture, pas un handicap.
func test_three_manuals_each_receive_the_whole() -> void:
	var scene := await _kill_scene()
	var player: Player = scene[0]
	var manager: EnemyManager = scene[1]
	var grunt: Enemy = scene[2]
	...
	for book in books:
		assert_eq(book.manual.experience, player.xp, "chacun le montant entier")
...
func test_the_character_gains_exactly_what_it_used_to() -> void:
	var scene := await _kill_scene()
	var player: Player = scene[0]
	var manager: EnemyManager = scene[1]
	var grunt: Enemy = scene[2]

	var expected := maxi(roundi(
		grunt.xp_value() * Enemy.experience_factor(manager.level, player.level)
	), 1)
	player.rack.put(0, _book())
	manager.report_kill(grunt)
	# Une image avant de conclure : la chute d'un objet passe par
	# `add_child.call_deferred()` (invariant 4), et sans elle le GroundItem est
	# créé sans jamais entrer dans l'arbre — donc jamais libéré.
