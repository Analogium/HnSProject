...
## suite d'entiers à partir de zéro, et l'indexation directe évite un accès
## haché à chaque coup porté.
...
## de dégâts qui s'envole, la gerbe d'éclats, la ligne de résistance de la
## fiche. Une seule définition, sinon « le froid » finirait par être deux bleus
## différents selon l'endroit où on le regarde.
##
## Choisies pour se distinguer en 32 px par la **valeur** autant que par la
## teinte — c'est la leçon du Colossal ambre confondu avec l'élite dorée. Aucune
## n'approche l'or, qui reste réservé aux critiques et aux élites.
const COLORS := [
