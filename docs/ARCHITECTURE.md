...
| Comment un ennemi prévient-il de son attaque ? | (jalon 27) Par une **`DangerZone`** au sol, un seul signal pour toutes les attaques, choisi sur planche : le plein monte du départ jusqu'au bord, et **quand il l'atteint, elle frappe elle-même** — l'obus tombe même si le mortier est mort. Armée par `Enemy._danger()` — nature, dégâts de la fiche et auteur ; le coup au contact du grunt et du chevalier, par `Enemy._hurt()`. Trois formes : disque (obus du mortier, éclatement du gonfle), couloir (charge du chevalier, qui ne frappe pas : c'est la charge qui touche) et cône (colosse). Un disque frappe par `Explosion.put()` — le souffle dessiné de sa nature, feu ou nécrose —, un cône par sa propre requête. Dessinée par `fx/danger_zone.gdshader`, **au pixel du monde** : le nœud ne tourne pas, la forme se calcule dans le sens de `facing`, et `DangerZone.contains()` en est le miroir côté jeu — ce qui est dessiné est ce qui est frappé. Liée à son ennemi (`bound`), elle tombe avec lui. Posée dans `EnemyManager.ground()` — le nœud `Ground` de la zone et de l'arène, entre le sol et les corps —, en différé : le gonfle la pose en mourant |
| Ce qui survit à la fermeture ? | `Character.to_dict()` et `Settings.to_dict()` |
| Quelle touche déclenche une action ? | La table du moteur, jamais un `KEY_*` écrit dans un `if`. Les défauts sont dans `project.godot`, le choix du joueur dans `Settings.key_binds`, et `Keybinds` les réunit. `Keybinds.ACTIONS` liste ce qui se rebinde, avec le libellé de l'onglet ; `Keybinds.MOUSE_LABELS` les boutons de souris acceptés — les trois clics et les deux latéraux, jamais la molette |
...
| Combien de coups porte un lancer, si tout touche ? | `SkillStats.average_per_cast()` : projectiles × cibles × coups de la forme × `strikes_over_duration()` — **la fonction même qui compte les impulsions du nuage**. Une aura n'a que `average_per_second()`. Quand part chaque impulsion d'un geste qui dure — nuage, pilier, pulsation, vortex, trace de ruée, créatures du portail : `SkillStats.strikes_due()`, la n-ième à n périodes |
| Comment voiler les sorts ou les attaques ennemies ? | `Settings.veil(nœud, groupe)` **à la naissance du visuel** : l'alpha du `modulate` pris sur `spell_opacity` ou `enemy_attack_opacity` (curseurs des options, 0 à 100 %), et le groupe `SPELLS` / `ENEMY_ATTACKS`, que le setter reprend pour ce qui vit encore. Une explosion se range par son `mask` : celle qui frappe le joueur est ennemie. **Un nouveau visuel de compétence ou d'attaque doit l'appeler**, sinon il reste opaque. Le shader de `DangerZone` multiplie `COLOR` pour garder le `modulate` |
...
| Le zoom du panneau de l'arbre ? | **Progressif** : un facteur continu entre `PassiveTreePanel.MIN_ZOOM` (0,35) et `MAX_ZOOM` (1,5), que chaque cran de molette multiplie par `ZOOM_STEP` (1,1) — un pavé tactile par fractions de cran. Il multiplie `UNIT` dans `screen_position()` et les rayons dans `_radius()` — donc la tolérance de `node_at()`. `zoom_by()` garde le centre du cadre. Pas d'icône sous `ICON_ZOOM` (0,75) ; retour à 1 à l'ouverture |
| La recherche de l'arbre ? | `PassiveTreePanel.search()` / `_find()`, le champ en haut à droite : **chaque mot** doit se trouver, sans casse, dans `displayed_name()` ou dans les lignes telles que l'info-bulle les écrit (`Glossary.plain(StatMod.label())`). Calculée à la frappe dans `_found`, jamais au dessin. Le reste s'éteint à `UNFOUND_ALPHA`, ce qu'on trouve prend un halo. Tant qu'un `LineEdit` a le focus, `Player._physics_process()` ne marche pas : ZQSD s'y écrit ; un clic dans l'arbre ou Entrée rend les touches |
