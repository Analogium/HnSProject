...
| Une ligne d'objet est-elle locale ? | `ItemBase.is_local()` : la chance critique d'une arme, plate (`cruel`, « +4 % de chance critique de base ») ou accrue (`precise`) ; **la défense d'une pièce d'armure** — l'armure ou l'esquive de son implicite (`ItemBase.defense_stat()`), plate (`cuirassed`, `elusive`) ou accrue (`plated`). `Item.mods()` ne les verse pas à la fiche ; elles font `Item.crit_chance()` et `Item.defense()` (plats, puis accrus, arrondi), que `mods()` verse chacune en une seule ligne plate — l'implicite de défense n'y entre pas à part. L'infobulle l'écrit « (local) » par `Item.explicit_line()`, et la base en propriété d'en-tête |
