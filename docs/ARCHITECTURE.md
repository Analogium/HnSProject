...
| Qu'est-ce qu'on peut lancer ? | `Player.cast_slot()`, qui porte les six refus — case vide, non apprise, mauvaise arme, réserve, recharge, orbite pleine. Un **geste entretenu** allumé — l'aura, un buff — s'y **éteint** sans coût, et la touche tenue ne le rallume pas |
...
| Ce qu'un buff allumé change ? | `Skill.lines`, **les lignes d'un passif** par point placé, versées par `Player.buff_mods()` dans la même liste que les objets portés. Un buff ne frappe pas : ses nombres ne sont pas dans `SkillStats`. Le nœud `Buff` porte le prix et le dessin, et **une durée de vie facultative** — zéro tant qu'on l'entretient, deux secondes pour celui qu'une ruée laisse |
